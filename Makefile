...
hdd:
	dd if=/dev/zero of=build/hdd.img bs=1M count=64

run:
	qemu-system-i386 \
		-drive format=raw,file=build/final.img,if=ide,index=0 \
		-drive format=raw,file=build/hdd.img,if=ide,index=1 \
		-m 32 -no-reboot

run-hdd:
	qemu-system-i386 -drive format=raw,file=build/hdd.img,if=ide -m 32 -no-reboot

clean:
	rm -rf build

.PHONY: all build mkfs hdd run run-hdd clean