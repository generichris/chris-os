...
static void net_irq_handler(struct registers regs) {
    (void)regs;
    eth_handle_interrupt();
}
...
void kernel(void) {
    ...
    vga_debug('I'); sched_init();
    vga_debug('J'); serial_init();
    vga_debug('K'); syscall_init();
    ...
    if (eth_ready()) {
        netstack_init();
        /* Use IRQ number read from PCI config space, not a hardcoded guess */
        uint8_t nic_irq = eth_get_irq();
        kprintf("net: installing IRQ handler for IRQ %d\n", nic_irq);
        irq_install_handler(nic_irq, net_irq_handler);
        terminal_writestring("net: stack up, running DHCP...\n");
        dhcp_request();
    } else {
        terminal_writestring("net: no NIC found (add -net nic,model=rtl8139 to QEMU)\n");
    }
    ...
    if (fb_active) {
        draw_desktop();
        draw_taskbar();
        draw_window(fb_width - 280, 40, 250, 100, "ChrisOS v0.7");
    ...
    for (;;) {
        ...
        if (fb_active) {
            ...
            if (now - last_draw_tick < 10) continue;
            last_draw_tick = now;
